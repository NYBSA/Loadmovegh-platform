...
export default function TopNavbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 sm:px-6 transition-colors">
            ...
            <div className="absolute right-0 top-full mt-2 w-56 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-2xl overflow-hidden">
              ...
              <div className="py-1">
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
                >
                  <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
              ...
              <div className="border-t border-gray-100 dark:border-gray-800 py-1">
                <button
                  ...
                  className="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition"
                >
      ...
      {/* ── Profile Modal ─────────────────────────────── */}
      {profileModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onClick={() => setProfileModalOpen(false)}>
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="relative bg-gradient-to-r from-brand-600 to-emerald-600 px-6 py-8 text-center">
              <button onClick={() => setProfileModalOpen(false)} className="absolute top-3 right-3 text-white/70 hover:text-white transition">
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
              </button>
              <div className="h-20 w-20 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-2xl font-bold text-white mx-auto shadow-lg border-2 border-white/30">
                {user?.name?.split(" ").map((n) => n[0]).join("").slice(0, 2) || "SA"}
              </div>
              <h2 className="mt-3 text-xl font-bold text-white">{user?.name || "System Admin"}</h2>
              <p className="text-sm text-white/80">{(user?.role || "admin").replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase())}</p>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium">Email</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white mt-0.5">{user?.email || "admin@loadmovegh.com"}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium">Role</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white mt-0.5">{(user?.role || "admin").replace("_", " ").replace(/\b\w/g, (c) => c.toUpperCase())}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium">Status</p>
                ...
                <div>
                  <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium">Last Login</p>
                  <p className="text-sm font-semibold text-gray-900 dark:text-white mt-0.5">Today, {new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                </div>
              </div>

              <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
                <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium mb-2">Permissions</p>
                <div className="flex flex-wrap gap-1.5">
                  {["User Management", "Fraud Review", "Revenue Access", "Load Analytics", "Commission Tracking", "Compliance", "Settings"].map((p) => (
                    <span key={p} className="inline-flex items-center rounded-md bg-brand-50 dark:bg-brand-900/20 px-2 py-0.5 text-[10px] font-medium text-brand-700 dark:text-brand-400 ring-1 ring-brand-200 dark:ring-brand-800">{p}</span>
                  ))}
                </div>
              </div>

              <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
                <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-medium mb-2">Recent Activity</p>
                <div className="space-y-2">
                  {[
                    { action: "Approved user Kofi Transport Ltd", time: "2 minutes ago" },
                    { action: "Investigated fraud alert #FA-003", time: "15 minutes ago" },
                    { action: "Exported commission CSV", time: "1 hour ago" },
                    { action: "Logged in from Accra, Ghana", time: "Today, 08:00 AM" },
                  ].map((a, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <div className="h-1.5 w-1.5 rounded-full bg-brand-400 flex-shrink-0" />
                      <p className="text-xs text-gray-700 dark:text-gray-300 flex-1">{a.action}</p>
            ...
            {/* Footer */}
            <div className="px-6 py-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 flex justify-end gap-2">
              <button onClick={() => setProfileModalOpen(false)} className="btn-secondary text-sm">Close</button>
              <button onClick={() => { setProfileModalOpen(false); setActiveSection("settings"); }} className="btn-primary text-sm">Edit Profile</button>
            </div>
          </div>
        </div>
      )}
